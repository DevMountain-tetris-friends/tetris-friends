...
function TetrisPage() {
    ...
    const [Tetris, setTetrisObject] = useState(null)
    ...
    useEffect(() => {
        if(!Tetris){
            setTetrisObject({
                config: {
                  ...
                  brick: "<b></b>",
                  freeBrick: {
                    background: "#000",  // <---- Grid Color
                    border: "1px solid #333"
                  },
                  filledBrick: {
                    background: "#fff",  // <---- Brick Color
        ...
        } else if (Tetris && firstLoaded){
            Tetris.init()
            setFirstLoaded(true)
        }
    }, [Tetris])

    console.log('got here to the tetris page')
    return(
        <div>
            <div id="tetris">
                <span id="start-btn">PLAY</span>
            </div>
            <h1>TETRIS!</h1>
            <h2>Use Arrows
                <ul>Right arrow: <i class="arrow right"></i></ul>
                <ul>Left arrow: <i class="arrow left"></i></ul>
                <ul>Up arrow: <i class="arrow up"></i></ul>
                <ul>Down arrow: <i class="arrow down"></i></ul>
            </h2>
        </div>
    )
